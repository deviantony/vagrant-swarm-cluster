...
docker -H 10.0.7.10:2375 run -d --restart always -p 4000:4000 --name swarm_manager --label type=swarm \
...
docker -H 10.0.7.11:2375 run -d --restart always --name swarm_node1 --label type=swarm \
swarm:${SWARM_VERSION} join --heartbeat 20s --ttl 30s --advertise 10.0.7.11:2375 consul://10.0.7.11:8500

echo "Starting Swarm node #2..."
docker -H 10.0.7.12:2375 run -d --restart always --name swarm_node2 --label type=swarm \
