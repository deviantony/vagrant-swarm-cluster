...
rm -rf /etc/docker/key.json
echo 'DOCKER_OPTS="-H tcp://0.0.0.0:2375 -H unix:///var/run/docker.sock --cluster-store=consul://10.0.7.10:8500 --cluster-advertise=eth1:2375"' | tee -a /etc/default/docker;
curl -fsSL https://raw.githubusercontent.com/CWSpear/local-persist/master/scripts/install.sh | bash -s -- --upstart;
ln -s /lib/init/upstart-job /etc/init.d/docker-volume-local-persist;
update-rc.d docker-volume-local-persist defaults;
service docker start;
SCRIPT
